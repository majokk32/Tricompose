"""Generate protected CXR candidates with frozen CheXGenBench Sana.

The job consumes only an existing protected EHR-fact selection. It never loads
a matched real CXR or source report. GPU execution is allowed only through Slurm.
"""
...
from .prompting import CLINICAL_PROMPT_VERSION, PROMPT_VERSION, build_sana_prompt
...
SUMMARY_SCHEMA = "tricompose.chexgenbench_sana.summary.v1"
SEED_OFFSET = 61_083_271
...
def _stage_cases(
    *, source_run_id: str, output_run_id: str, limit: int
    ...
    for source_record in source_records[:limit]:
        ...
        prompt = build_sana_prompt(facts)
        ...
        write_private_json(
            case_dir / "input.json",
            {
                "schema_version": "tricompose.chexgenbench_sana.case_input.v1",
                "case_id": case_id,
    ...
    write_private_json(
        output_run / "manifest.json",
        {
            ...
            "producer": "frozen_chexgenbench_sana_e20",
...
def run(
    *,
    ...
    peak_vram = 0.0
    protected_log = output_run / "logs" / "sana.log"

    try:
        with protected_log.open("x", encoding="utf-8") as log_handle:
            with contextlib.redirect_stdout(log_handle), contextlib.redirect_stderr(log_handle):
                runtime = FrozenSanaRuntime(model_dir)
                for batch_ordinal, batch in enumerate(_chunks(records, batch_size)):
                    ...
                    result = runtime.generate_batch(
                        [str(item["sana_prompt"]) for item in inputs],
                    ...
                    for row, case_input, image, token_count in zip(
                        ...
                        record_path = write_private_json(
                            generation_dir / "generation.json",
                            {
                                ...
                                "case_id": case_id,
                                "candidate_id": "chexgenbench_sana_e20_seed0",
                                "modality": "cxr",
                                "producer": "frozen_chexgenbench_sana_e20",
                                "frozen_model": True,
        ...
        frozen_path = write_private_json(
            ...
            {
                "schema_version": "tricompose.chexgenbench_sana.frozen_run.v1",
                "status": "frozen",
                "run_id": output_run_id,
