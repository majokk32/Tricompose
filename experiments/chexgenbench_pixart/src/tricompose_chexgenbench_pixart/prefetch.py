"""Download and validate the pinned public CheXGenBench PixArt snapshot."""
...
def prefetch(destination: str | Path, staging_dir: str | Path) -> dict[str, object]:
    ...
    if destination.exists():
        raise FileExistsError("final PixArt snapshot already exists")
    if staging_dir.exists():
        raise FileExistsError("PixArt staging directory already exists")
