...
import tricompose_v11.report_execution as execution


class V11ReportExecutionCleanupTests(unittest.TestCase):
    def test_preexisting_target_is_never_deleted_on_failure(self) -> None:
        with tempfile.TemporaryDirectory(prefix="tricompose_v11_report_exec_") as temp:
            root = Path(temp)
            ...
            with (
                ...
                mock.patch.object(execution, "load_model_requests", return_value=[]),
            ):
                with self.assertRaises(FileExistsError):
                    execution.run_report_request_run(
                        request_run=request,
                        output_root=output,
