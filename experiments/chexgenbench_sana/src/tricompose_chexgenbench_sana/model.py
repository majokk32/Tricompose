"""Audited, frozen and local-only CheXGenBench Sana runtime."""
...
OFFICIAL_MODEL_REPO = "raman07/CheXGenBench-Models-Sana-e20"
OFFICIAL_MODEL_REVISION = "6c8989d92e75fbcfc91a82c3570389293867ad6a"
...
EXPECTED_MODEL_INDEX_SHA256 = (
    "f2c25169ee98af8964867731386345d89772f8bfb00d9e712f4398b6eb79f201"
)
EXPECTED_WEIGHT_SIZES = {
    "text_encoder/model-00001-of-00002.safetensors": 4_988_024_144,
    "text_encoder/model-00002-of-00002.safetensors": 240_691_624,
    "transformer/diffusion_pytorch_model.safetensors": 2_367_054_536,
    "vae/diffusion_pytorch_model.safetensors": 1_249_044_836,
}
...
def validate_model_snapshot(model_dir: str | Path) -> dict[str, Any]:
    """Fail closed unless the pinned public snapshot is complete."""

    root = Path(model_dir).resolve(strict=True)
    if not root.is_dir():
        raise ValueError("Sana model path is not a directory")
    ...
    if payload.get("_class_name") != "SanaPipeline":
        raise ValueError("snapshot is not a SanaPipeline")
    if payload.get("text_encoder") != ["transformers", "Gemma2Model"]:
...
class FrozenSanaRuntime:
    """Load the official pipeline without network access or parameter updates."""
    ...
    def _assert_frozen(self) -> None:
        ...
        for name, component in self.pipe.components.items():
            ...
            if component.training:
                raise RuntimeError(f"Sana component entered training mode: {name}")
            if any(parameter.requires_grad for parameter in component.parameters()):
                raise RuntimeError(f"Sana component is not frozen: {name}")
    ...
    def generate_batch(
        ...
        if any(count > DEFAULT_MAX_SEQUENCE_LENGTH for count in counts):
            raise ValueError("protected prompt exceeds Sana tokenizer limit")
        ...
        if len(images) != len(prompts):
            raise RuntimeError("Sana returned an unexpected image count")
        return BatchResult(
            images=images,
