"""Protected-path and identifier helpers for the EHR-prompt CXR baselines."""
...
PROTECTED_EXPERIMENT_ROOT = PROTECTED_ROOT
RUN_ID_PATTERN = re.compile(r"[A-Za-z0-9][A-Za-z0-9._-]{0,63}")
CASE_ID_PATTERN = re.compile(r"case_[0-9]{3}")
SUPPORTED_MODELS = frozenset({"unidisc", "liquid"})
...
def create_output_run(model_name: str, run_id: str) -> Path:
    validate_model_name(model_name)
    validate_run_id(run_id)
    return create_private_stage_dir(
        PROTECTED_EXPERIMENT_ROOT / model_name / "runs" / run_id
    )
