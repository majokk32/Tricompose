#!/usr/bin/env python3
"""Run one frozen V1.1 CXR-to-report expert from exact requests (Slurm only)."""
...
from tricompose_v11.report_execution import run_report_request_run  # noqa: E402
...
def _model_components(args):
    if args.precision is not None and args.model_id != "chexagent2":
        raise ValueError("--precision is only valid for CheXagent-2")
    if args.model_id == "maira2":
        ...
        return (
            MODEL_REVISION,
            validate_model_snapshot(args.model_dir),
            lambda: FrozenMaira2Runtime(args.model_dir),
        )
    if args.model_id == "cxrmate_single":
        ...
        return (
            MODEL_REVISION,
            validate_model_snapshot(args.model_dir),
            lambda: FrozenCXRMateSingleRuntime(args.model_dir),
        )
    if args.model_id == "llavarad":
    ...
    if args.model_id == "chexagent2":
        ...
        selected_precision = args.precision or "bfloat16"
        audit = {
            "model": validate_model_snapshot(args.model_dir),
            "vision": validate_vision_snapshot(vision_dir),
            "runtime_precision": selected_precision,
        }

        def factory():
            runtime = FrozenCheXagent2Runtime(
                model_dir=args.model_dir,
    ...
    raise ValueError("unsupported V1.1 report model")
